...
const LocationandRideM = ({ selectedVehicle, setSelectedVehicle, VEHICLES, setisRiding, pannelRef }) => {
    ...
    useEffect(() => {
        ...
        gsap.to(pannelRef.current, {
            height: showSuggestions ? 'min(86vh, 34rem)' : '13.8rem',
            duration: 0.35,
            ease: 'power3.out',
            overwrite: true,
        })
    ...
    return (
        <div className='mx-2 mb-2 overflow-hidden rounded-[28px] border border-white/70 bg-white/90 shadow-[0_20px_70px_rgba(15,23,42,0.25)] backdrop-blur-xl'>
            <div className='border-b border-slate-200/80 bg-white/80 px-4 py-3'>
                <div className='flex items-start justify-between gap-3'>
                    <div>
                        <h1 className='text-lg font-bold tracking-tight text-slate-900'>Find a trip</h1>
                        <p className='mt-1 text-xs font-medium text-slate-500'>Choose your pickup and drop-off points</p>
                    </div>
                    ...
                    <button
                        ...
                        className={`flex h-9 w-9 items-center justify-center rounded-full border border-slate-200 bg-slate-50 text-slate-700 transition-all duration-300 hover:bg-slate-100 ${showSuggestions ? 'rotate-0' : 'rotate-180'}`}
                    >
            ...
            <form onSubmit={submitHandler} className={`flex h-full flex-col gap-3 bg-transparent ${selectVehicle ? 'p-2' : 'p-3'}`}>
                {!selectVehicle ? (
                    <>
                        {/* Inputs */}
                        <div className='relative flex flex-col gap-3'>
                            <div className='absolute left-4.5 top-6 z-10 bottom-6 w-0.5 rounded-full bg-slate-800/70' />
                        ...
                        {/* Suggestions */}
                        <div
                            className='rounded-2xl border border-slate-200 bg-white/90 shadow-sm overflow-y-auto scrollbar-thin'
                            style={{
                                maxHeight: showSuggestions ? '13.5rem' : 0,
                        ...
                        <div
                            ...
                            style={{
                                maxHeight: showSuggestions ? '6rem' : 0,
                                opacity: showSuggestions ? 1 : 0,
                                transition: 'max-height 0.35s ease, opacity 0.35s ease',
                            }}
                        >
                            ...
                            <button
                                ...
                                className='mt-3 flex w-full items-center justify-center gap-2 rounded-2xl bg-slate-900 py-3.5 text-sm font-semibold text-white shadow-lg shadow-slate-900/20 transition-all duration-200 hover:bg-black active:scale-[0.99]'
                            >
                                <IoMdSearch size={18} />
                    ...
                    <div className='bg-white border border-gray-200/90 shadow-xl shadow-slate-200/50 rounded-2xl flex flex-col overflow-hidden max-h-full'>
                        ...
                        {/* Vehicles List */}
                        <div className='p-2 overflow-y-auto scrollbar-thin flex flex-col gap-2.5 divide-y divide-slate-100/60' style={{ maxHeight: '14rem' }}>
                                ...
                                return (
                                    ...
                                    >
                                        <div className='flex items-center gap-2 flex-1 min-w-0'>
                                            <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${isSelected ? 'bg-slate-800 text-white' : 'bg-slate-100 text-slate-800'
                                                }`}>
                                                <Icon size={20} />
                                            </div>
                                            ...
                                            <div className='flex-1 min-w-0 pr-2'>
                                                <div className='flex items-center gap-2'>
                                                    ...
                                                    <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-md ${isSelected ? 'bg-slate-700 text-slate-200' : 'bg-slate-100 text-slate-600'} flex justify-center items-center`}>
                                                        <FaUser size={7} className='inline mr-1' />
                                                        {vehicle.capacity}
                                                    </span>
                                                    {vehicle.badge && (
                                                        <span className={`text-[7px] font-bold uppercase px-1.5 py-0.5 rounded-full ${isSelected ? 'bg-emerald-500 text-slate-950' : 'bg-emerald-50 text-emerald-800'
                                                            }`}>
                                                            {vehicle.badge}
                                                        </span>
                                                    )}
                                        ...
                                        <div className='text-right shrink-0'>
                                            <span className={`text-sm font-extrabold block ${isSelected ? 'text-white' : 'text-slate-900'}`}>
                                                {vehicle.price}
                                            </span>
                                        </div>
                                    </button>
...
export default LocationandRideM